...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  // Bloqueio por falta de licença ativa (assinatura)
  ...
  Future<void> _checkAndLaunchPlayer() async {
    // Verificar licença: sem assinatura ativa não inicia reprodução
    if (LicenseConfig.isConfigured) {
      final result = await LicenseService.instance.checkLicense();
      if (!mounted) return;
      if (!result.isActive) {
        setState(() => _licenseBlocked = true);
        return;
      }
    }
  ...
  void _initFlutterPlayer() {
    ...
    // Modo imersivo (tela inteira) + landscape no celular ao iniciar vídeo
    SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky);
    if (PlatformDetector.isMobile) {
  ...
  void dispose() {
    ...
    // Restore system UI e orientação (celular)
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: _licenseBlocked ? _buildLicenseBlockOverlay() : Focus(
  ...
  Widget _buildLicenseBlockOverlay() {
        ...
        child: Center(
          child: Padding(
            ...
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(Icons.lock_outline, size: 72, color: primaryColor.withOpacity(0.9)),
                ...
                Text(
                  'Ative seu acesso VIP para reproduzir este conteúdo.',
                ...
                if (EnvConfig.isStripeConfigured) ...[
                  FilledButton.icon(
                    onPressed: _checkoutLoading ? null : () async {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // Botão cadeado - travar controles ao assistir
          const SizedBox(width: 8),
          TVFocusable(
            onSelect: () {
              setState(() {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Consumer<SettingsProvider>(
                builder: (context, settings, _) {
                  final showSeekButtons = provider
                      .shouldShowProgressBar(settings.progressBarMode);
                  return Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      ...
                      // Retroceder 10s (conteúdo seekable)
                      if (showSeekButtons)
                        TVFocusable(
                          onSelect: () => provider.seekBackward(10),
                          focusScale: 1.0,
                          showFocusBorder: false,
                          builder: (context, isFocused, child) {
                            return Container(
                      ...
                      if (showSeekButtons) const SizedBox(width: 8),
                  ...
                  // Avançar 10s (conteúdo seekable)
                  if (showSeekButtons) const SizedBox(width: 8),
                  if (showSeekButtons)
                    TVFocusable(
                      onSelect: () => provider.seekForward(10),
                      focusScale: 1.0,
                      showFocusBorder: false,
                      builder: (context, isFocused, child) {
                        return Container(
