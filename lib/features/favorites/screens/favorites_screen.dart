...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Desktop: não duplicar TVSidebar (a Home já tem); tela cheia mostra AppBar com voltar.
    if (isTV) {
      return Scaffold(
        appBar: widget.embedded
            ...
            : AppBar(
                ...
                title: Text(
                  AppStrings.of(context)?.favorites ?? 'Favoritos',
  ...
  Widget _buildContent(BuildContext context) {
    return Consumer<FavoritesProvider>(
      builder: (context, provider, _) {
        ...
        if (provider.favorites.isEmpty && provider.vodFavorites.isEmpty) {
          return _buildEmptyState();
        }

        return _buildFavoritesContent(provider);
      },
    );
  }

  Widget _buildFavoritesContent(FavoritesProvider provider) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (provider.vodFavorites.isNotEmpty) ...[
            Padding(
            ...
            _buildVodFavoritesGrid(provider),
  ...
  Widget _buildVodFavoritesGrid(FavoritesProvider provider) {
    return GridView.builder(
      ...
      itemCount: provider.vodFavorites.length,
      itemBuilder: (context, index) {
        final v = provider.vodFavorites[index];
        final streamId = v['stream_id']?.toString() ?? '';
        final name = v['name']?.toString() ?? '';
        final iconUrl = v['icon_url']?.toString();
        final type = v['stream_type']?.toString() ?? 'movie';
        final xtreamStream = XtreamStream(
          streamId: streamId,
          name: name,
          streamIcon: iconUrl,
          containerExtension: v['container_extension']?.toString(),
        );
        return _buildVodFavoriteCard(provider, xtreamStream, type);
      },
    );
  }

  Widget _buildVodFavoriteCard(FavoritesProvider provider, XtreamStream item, String type) {
      ...
      onTap: () {
        if (type == 'series') {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => SeriesDetailScreen(series: item, tmdbData: null),
            ),
          );
        } else {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => MovieDetailScreen(movie: item, tmdbData: null),
            ),
          );
        }
      },
        ...
        children: [
          ...
          Positioned(
            ...
            child: IconButton(
              ...
              onPressed: () => provider.removeVodFavorite(item.streamId),
