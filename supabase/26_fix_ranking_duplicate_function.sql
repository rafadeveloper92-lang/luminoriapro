-- =============================================================================
-- CORREÇÃO: Existem duas funções get_global_ranking (migração 12 e 24), o que
-- causa erro PGRST203 "Multiple Choices". Remove todas e recria uma só.
-- Execute no Supabase SQL Editor e abra o ranking no app de novo.
-- =============================================================================

-- Remove as duas versões (ordem de parâmetros diferente no BD)
drop function if exists public.get_global_ranking(text, int, int);
drop function if exists public.get_global_ranking(int, int, text);

-- Recria uma única função (todos os usuários, ordenados por horas no mês)
...
comment on function public.get_global_ranking(int, int, text) is 'Ranking global: todos os usuários cadastrados, ordenados por horas assistidas no mês';
