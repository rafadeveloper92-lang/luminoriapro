...
void main() async {
  try {
    ...
    // Carrega .env (em assets) — pode rodar "flutter run" direto, sem script. Se .env não existir, não quebra.
    try {
    ...
    try {
      if (LicenseConfig.isConfigured) {
        await Supabase.initialize(
          url: LicenseConfig.supabaseUrl,
          anonKey: LicenseConfig.supabaseAnonKey,
        );
      }
    } catch (_) {}
    ...
    return Consumer<SettingsProvider>(
      builder: (context, settings, _) {
        ...
        return MaterialApp(
          ...
          title: AppStrings.of(context)?.lotusIptv ?? 'Luminoria',
