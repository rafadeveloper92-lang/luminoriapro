...
class _AddPlaylistDialogState extends State<AddPlaylistDialog> {
  ...
  void initState() {
    ...
    _usernameFocusNode = FocusNode(debugLabel: 'dialog_playlist_username');
    _passwordFocusNode = FocusNode(debugLabel: 'dialog_playlist_password');
  }
  ...
  /// Builds URL with optional HTTP Basic Auth (username:password@host)
  String _buildUrlWithAuth(String url, String username, String password) {
    if (username.isEmpty && password.isEmpty) return url;
    try {
      final uri = Uri.parse(url);
      final auth = '${Uri.encodeComponent(username)}:${Uri.encodeComponent(password)}';
      return uri.replace(userInfo: auth).toString();
    } catch (_) {
      return url;
    }
  }
        ...
        return Dialog(
          ...
          child: Container(
            constraints: BoxConstraints(
              ...
              maxHeight: isLandscape ? 300 : 700, // Slightly increased height
            ),
            decoration: BoxDecoration(
              color: AppTheme.getBackgroundColor(context),
    ...
    return Column(
      ...
      children: [
        ...
        _buildSecondaryButton(
          onPressed: () => _navigateToXtreamLogin(context),
          icon: Icons.vpn_key_rounded,
          label: 'Login Xtream Codes',
        ),
        const SizedBox(height: 12),
        Row(
          children: [
  ...
  Widget _buildTVContent(PlaylistProvider provider) {
    return Column(
      ...
      children: [
        _buildImportCard(
          onPressed: () => _showM3uUrlFormForTv(context, provider),
          icon: Icons.link_rounded,
          title: AppStrings.of(context)?.addFromUrl ?? 'Add from URL',
          subtitle: 'M3U/M3U8 URL com usuário e senha',
          isPrimary: true,
        ),
        const SizedBox(height: 12),
        _buildImportCard(
  ...
  Future<bool> _addPlaylist(PlaylistProvider provider, {bool popOnSuccess = true}) async {
    final name = _nameController.text.trim();
    var url = _urlController.text.trim();
    final username = _usernameController.text.trim();
    final password = _passwordController.text.trim();

    url = _buildUrlWithAuth(url, username, password);
  ...
  Future<void> _showM3uUrlFormForTv(BuildContext context, PlaylistProvider provider) async {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      final success = await _addPlaylist(provider, popOnSuccess: false);
                      if (success && mounted) {
                    ...
                    child: Text(AppStrings.of(context)?.addFromUrl ?? 'Add'),
                  ),
                ],
              ),
            ],
  ...
  Future<void> _navigateToXtreamLogin(BuildContext context) async {
    // Navigate to Login Screen
    // wait for result
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const XtreamLoginScreen()),
    );
    
    // Check if login was successful
    if (result == true && mounted) {
      // Close Add Playlist Dialog
      Navigator.pop(context, true);
    }
  }
