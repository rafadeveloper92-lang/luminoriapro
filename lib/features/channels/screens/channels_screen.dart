...
class _ChannelsScreenState extends State<ChannelsScreen> {
  ...
  int? _selectedChannelIndex; // canal selecionado na lista (destaque estilo IPTV Smarters)
      ...
      builder: (context, provider, _) {
        ...
        return CustomScrollView(
          ...
          slivers: [
            ...
            // Lista de canais (estilo IPTV Smarters): lista rolável, sem grid de miniaturas
            if (channels.isEmpty)
              SliverFillRemaining(
                child: Center(
                  child: Column(
              ...
              SliverPadding(
                padding: EdgeInsets.only(
                  left: isMobile ? (isLandscape ? 4 : 8) : 12,
                  right: isMobile ? (isLandscape ? 4 : 8) : 12,
                  top: isMobile ? (isLandscape ? 4 : 8) : 12,
                  bottom: isMobile ? (isLandscape ? 4 : 8) : 12,
                ),
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      final isLastRow = index == channels.length - 1;
                      ...
                      return ChannelListRow(
                        ...
                        onFocused: PlatformDetector.isTV ? () { _lastChannelIndex = index; } : null,
                        onLeft: (PlatformDetector.isTV && isFirstRow)
                            ? () {
  ...
  Future<void> _handleChannelTap(BuildContext context, Channel channel) async {
    ...
    if (settingsProvider.enableMultiScreen) {
      if (PlatformDetector.isTV && PlatformDetector.isAndroid) {
        ...
        multiScreenProvider.playChannelAtDefaultPosition(channel, settingsProvider.defaultScreenPosition);
        Navigator.pushNamed(context, AppRouter.player, arguments: {'channelUrl': '', 'channelName': '', 'channelLogo': null});
      } else {
        Navigator.pushNamed(context, AppRouter.player, arguments: {'channelUrl': channel.url, 'channelName': channel.name, 'channelLogo': channel.logoUrl});
      }
    } else {
      Navigator.pushNamed(context, AppRouter.player, arguments: {'channelUrl': channel.url, 'channelName': channel.name, 'channelLogo': channel.logoUrl});
    }
